  ...
  $(document).ready(function () {
    ...
    $("#user_avatar_input").on("change", function (e) {
      if (e.target.files[0]) {
        ...
        if( typeimage == false) {
            toastr.info('HÃ¬nh áº£nh khÃ´ng Ä‘Ãºng Ä‘á»‹nh dáº¡ng png hoáº·c jpeg !');
            return;
        }
        if(file.size > 1048576){
            toastr.info('KÃ­ch thÆ°á»›c hÃ¬nh áº£nh pháº£i nhá» hÆ¡n 1MB !');
            return;
        }
    ...
    $(".product-detail__quantity-input").each(function () {
      ...
      incBtn.on("click", function (e) {
        if (size.val() == '') {
          toastr.info('Báº¡n chÆ°a chá»n size !');
        } else if (size.val() == 0) {
            toastr.info('Sáº£n pháº©m Ä‘Ã£ háº¿t hÃ ng Online. Báº¡n cÃ³ thá»ƒ "TÃ¬m táº¡i cá»­a hÃ ng" !');
        } else {
      ...
      $.ajax({
        ...
        success: function (response) {
          if(response.status == false) {
            ...
            toastr.success('Báº¡n thay Ä‘á»•i máº­t kháº©u thÃ nh cÃ´ng !');
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response.response_type == 'success'){
                    toastr.success('Báº¡n Ä‘Äƒng kÃ½ thÃ nh cÃ´ng !');
                }else{
    ...
    $(document).on('click', '#popup_btn_subscribe', function () {
        ...
        $.ajax({
            ...
            success: function (response) {
                if(response.response_type == 'success'){
                    $.fancybox.close();
                    toastr.success('Voucher Ä‘Ã£ Ä‘Æ°á»£c gá»­i vá» email cá»§a báº¡n !');
                }else{
                    toastr.info(response.response_msg);
                }
                clearBusy();
    ...
    $(document).on('click', '.btn-copy-voucher', function () {
        ...
        $temp.remove();
        toastr.success('Thu tháº­p mÃ£ thÃ nh cÃ´ng !');
    });
    ...
    $('.get-otp-login').on('click', function () {
        ...
        if (!customer_account) {
            toastr.warn('TÃªn Ä‘Äƒng nháº­p khÃ´ng há»£p lá»‡ !');
  ...
  $scope.lis_stats = function(status, payment_method){
      if (status == 20){
          if(status != 1 && payment_method != 3){
              return 'Äáº·t hÃ ng thÃ nh cÃ´ng';
          }else {
            return 'Äáº·t hÃ ng thÃ nh cÃ´ng';
          }
        ...
        return 'ÄÆ¡n Ä‘ang xá»­ lÃ½';
      }else if(status == 39 || status == 27 ||status == 28 || status == 49){
        return 'ÄÃ£ gá»­i';
      }else {
          if (status == 3){
              return  'Xuá»‘ng pháº§n má»m';
          } else if(status == 17){
              return 'ÄÃ£ Tráº£ hÃ ng/Há»§y COD';
          } else if(status == 5){
              return 'ÄÃ£ Tráº£ hÃ ng/Äá»•i hÃ ng';
          }else if(status == 1){
              return 'HoÃ n ThÃ nh';
          }else if(status == 2){
              return 'ÄÃ£ há»§y Ä‘Æ¡n hÃ ng';
          }else if(status == 31){
              return 'ÄÆ¡n HT';
          }else if(status == 43){
              return 'CH Ä‘Ã£ gÃ³i hÃ ng';
          }else if(status == 45){
              return 'ÄÆ¡n chuáº©n bá»‹ vk';
          }else if(status == 48){
              return 'ON1 xá»­ lÃ½';
          };
      }
