$(document).ready(function(){
    ...
    $('#but_subscribe').click(function(){
        ...
        {
            p_message('#subscribe_error', '<p class="required">Vui lÃ²ng nháº­p Ä‘á»‹a chá»‰ email há»£p lá»‡</p>');
        }else{
    ...
    $(document).on('click', '#but_customer_pay', function(){
        ...
        {
            p_message('#p_msg_alert', 'Vui lÃ²ng nháº­p sá»‘ tiá»n há»£p lá»‡ (>= 1000 vnÄ‘)');
            return false;
        }else{
    ...
    $('#but_cart_send').on('click', function(){
        ...
        $('#frm_cart_mail input').each(function(){
            ...
            if (el == '')
            {
                p_message('#p_cart_mail', '<span class="required">Vui lÃ²ng nháº­p Ä‘áº§y Ä‘á»§ cÃ¡c thÃ´ng tin yÃªu cáº§u</span>');
                check = false;
                return false;
            }
        });
    ...
    $('.but_order_cancel').click(function(){
        if (confirm('Báº¡n cháº¯n cháº¯n muá»‘n huá»· Ä‘Æ¡n hÃ ng nÃ y?')){
    ...
    $('.but-confirm-sms').click(function(){
        ...
        {
            alert('Vui lÃ²ng tick Ä‘á»ƒ xÃ¡c nháº­n Ä‘á»“ng Ã½.');
            return false;
        }
    });
    ...
    $('.comment_file').on('change', function () {
        ...
        if (count == 0) {
            ...
            $('.count_file').html($(this)[0].files.length + ' áº£nh Ä‘Ã£ Ä‘Æ°á»£c táº£i lÃªn');
        }
    });
    ...
    $('.btn-comment').on('click', function () {
       if (typeof logged_in_id == "undefined" || logged_in_id.length == 0) {
           toastr.info('Báº¡n cáº§n Ä‘Äƒng nháº­p Ä‘á»ƒ thá»±c hiá»‡n chá»©c nÄƒng nÃ y !');
           return;
       }
       ...
       switch (rating_number) {
        ...
        if (list_file.length > 5) {
            toastr.info('Tá»‘i Ä‘a 5 áº£nh trong 1 Ä‘Ã¡nh giÃ¡ !');
            clearBusy();
            return;
        }
        ...
        if (check_size_image == 1) {
            toastr.info('KÃ­ch thÆ°á»›c áº£nh tá»‘i Ä‘a 1MB !');
            $('.comment_file').val('');
            $('.count_file').html('');
            return;
        }
        ...
        $.ajax({
            ...
            if (response == true || typeof JSON.parse(response) == 'object') {
                ...
                $('.comment_file').val('');
                $('.count_file').html('');
                toastr.success('Cáº£m Æ¡n nhá»¯ng chia sáº» quÃ½ bÃ¡u cá»§a báº¡n. ChÃºng tÃ´i sáº½ ngÃ y cÃ ng hoÃ n thiá»‡n Ä‘á»ƒ mang tá»›i báº¡n nhá»¯ng sp vÃ  tráº£i nghiá»‡m tá»‘t hÆ¡n !');
            } else {
                clearBusy();
                toastr.info('ÄÃ£ cÃ³ lá»—i xáº£y ra !');
            }
        });
    });*/
    ...
    */
    ...
    $(document).on('click', '.copy-voucher', function () {
        ...
        $temp.remove();
        toastr.success('Copy Voucher thÃ nh cÃ´ng !');
    });
    ...
    $(document).on('change', '#find_shop_region_id', function(){
        ...
        if (value == 'Chá»n Tá»‰nh/ThÃ nh phá»‘') value = '';
        findAtTheShop(value, '', '');
    });
...
function process_check_showroom()
{
    ...
    if(real_ip_xxx == undefined){
        ...
        if (navigator.geolocation) {
            ...
            p_message('#msg_cuahang', 'Vui lÃ²ng cho phÃ©p chÃºng tÃ´i láº¥y thÃ´ng tin Ä‘á»‹a chá»‰.');
        } else {
            p_message('#msg_cuahang', 'TrÃ¬nh duyá»‡t khÃ´ng khá»— trá»£ láº¥y thÃ´ng tin Ä‘á»‹a chá»‰.');
            console.log('test-az');
        }
    ...
    switch (type) {
        ...
        case 'payment_method':
            ...
            if (paymentMethodOnlineList.includes(Number(paymentMethod)))
                $('button[name=btn_continue_step2]').text('TIáº¾P Tá»¤C THANH TOÃN');
            else
                $('button[name=btn_continue_step2]').text('HOÃ€N THÃ€NH');
            ...
            switch (type) {
                ...
                case 'update_address':
                    if (typeof response.entity.estimated_delivery_time != 'undefined')
                        $('.delivery-time').html('Thá»i gian giao hÃ ng dá»± kiáº¿n: ' + response.entity.estimated_delivery_time);
...
function changeInfoCheckout(type)
{
    ...
    {
        // $('#but_step2').text('TIáº¾P Tá»¤C THANH TOÃN');
        $('button[name=btn_continue_step2]').text('TIáº¾P Tá»¤C THANH TOÃN');
    }else{
        // $('#but_step2').text('HOÃ€N THÃ€NH');
        $('button[name=btn_continue_step2]').text('HOÃ€N THÃ€NH');
    }
    ...
    $.ajax({
        ...
        //     {
        //         p_message('#p_coupon', 'MÃ£ nhÃ¢n viÃªn há»£p lá»‡');
        //     }
        // }
        ...
        if (data.delivery_time !== false) {
            $('.delivery-time').html('Thá»i gian giao hÃ ng dá»± kiáº¿n: ' + data.delivery_time);
        }
...
function check_product(check_show_room)
{
    ...
    if (size == '' && color == '')
    {
        $('.vuilongchon').text('Vui lÃ²ng chá»n mÃ u sáº¯c, size');
        return false;
    }
    ...
    if (typeof (product_sub) != 'undefined')
    {
        ...
        if(typeof (obj_product[size_color]) !== 'undefined')
        {
            ...
            {
                $('.vuilongchon').html('Sáº£n pháº©m Ä‘Ã£ háº¿t hÃ ng Online.<br>&nbsp;Báº¡n cÃ³ thá»ƒ "TÃ¬m cá»­a hÃ ng".');
            ...
            if( color != '' && size == '')
            {
                $('.vuilongchon').text('Vui lÃ²ng chá»n size *');
            }else if (color == '' && size != ''){
                $('.vuilongchon').text('Vui lÃ²ng chá»n mÃ u *');
            }else{
                $('.vuilongchon').html('Sáº£n pháº©m Ä‘Ã£ háº¿t hÃ ng Online.<br>&nbsp;Báº¡n cÃ³ thá»ƒ "TÃ¬m cá»­a hÃ ng".');
                if(check_show_room) process_check_showroom();
            }
