$(document).ready(function () {
    $(document).on('click', '.color-picker', function () {
        ...
        $.ajax({
            ...
            if (response.product_stock_status == 3) {
                price_span = '<span class="product_stock">Chá» nháº­p</span>';
            } else {
                product.find('.add-to-cart').removeClass('hidden');
                if (response.coupon_price_sale != '') {
                    price_span = '<ins><span>' + response.coupon_price_sale + 'Ä‘</span></ins><del style="margin-left: 4px;"><span>' + response.product_price_format + 'Ä‘</span></del>';
                } else {
                    price_span = '<ins><span>' + response.product_price_format + 'Ä‘</span></ins>';
                }
            }
            ...
            if (response.product_quantity_tmp_total == 0 || !response.product_quantity_tmp_total) {
                ...
                //     product.append('<div class="out-of-stock">Háº¿t hÃ ng</div>');
                // }
            } else {
                if (bag_a.hasClass('bag-gray')) {
    ...
    $( "#slider-range").slider({
        ...
        slide: function( event, ui ) {
            $( "#amout-from" ).html(FormatCurrency(ui.values[0]) +'Ä‘');
            $( "#amout-to" ).html(FormatCurrency(ui.values[1]) + 'Ä‘');
    ...
    $( "#slider-range-mb").slider({
        ...
        values: [ product_price_from, product_price_to ],
        slide: function( event, ui ) {
            $( "#amout-from-mb" ).html(FormatCurrency(ui.values[0]) +'Ä‘');
            $( "#amout-to-mb" ).html(FormatCurrency(ui.values[1]) + 'Ä‘');
